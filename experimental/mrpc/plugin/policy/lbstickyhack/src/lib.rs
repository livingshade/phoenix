...
use crate::config::lbstickyhackConfig;
use crate::module::lbstickyhackAddon;

#[no_mangle]
pub fn init_addon(config_string: Option<&str>) -> InitFnResult<Box<dyn PhoenixAddon>> {
    let config = lbstickyhackConfig::new(config_string)?;
    let addon = lbstickyhackAddon::new(config);
    Ok(Box::new(addon))
}
