...
use phoenix_api_policy_lbstickyhack::control_plane;
...
pub(crate) struct lbstickyhackEngine {
    pub(crate) node: DataPathNode,
    pub(crate) indicator: Indicator,
    pub(crate) config: lbstickyhackConfig,
    pub(crate) lb_tab: HashMap<String, i32>,
}
...
impl Engine for lbstickyhackEngine {
    fn activate<'a>(self: Pin<&'a mut Self>) -> BoxFuture<'a, EngineResult> {
        Box::pin(async move { self.get_mut().mainloop().await })
    }

    fn description(self: Pin<&Self>) -> String {
        "lbstickyhackEngine".to_owned()
    }
    ...
    fn handle_request(&mut self, request: Vec<u8>, _cred: UCred) -> Result<()> {
        ...
        match request {
            control_plane::Request::NewConfig() => {
                self.config = lbstickyhackConfig {};
            }
        }
        Ok(())
    }
...
impl_vertex_for_engine!(lbstickyhackEngine, node);

impl Decompose for lbstickyhackEngine {
...
impl lbstickyhackEngine {
    pub(crate) fn restore(
        ...
        let config = *local
            .remove("config")
            .unwrap()
            .downcast::<lbstickyhackConfig>()
            .map_err(|x| anyhow!("fail to downcast, type_name={:?}", x.type_name()))?;
        let mut lb_tab = HashMap::new();
        let engine = lbstickyhackEngine {
            node,
            ...
            Ok(msg) => {
                match msg {
                    EngineTxMessage::RpcMessage(msg) => {
                        ...
                        let mut key = hello_HelloRequest_name_readonly(&rpc_req);
                        match (self.lb_tab.get(&key)) {
                            Some(dst) => {}
                            None => {
                                self.lb_tab.insert("name".to_string(), 1);
                            }
                        }
